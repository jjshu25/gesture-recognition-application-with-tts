...
class GestureRecognizerHelper(
    ...
    fun recognizeLiveStream(imageProxy: ImageProxy) {
        try {
            // Create bitmap with matching dimensions and config
            ...
            // Create IntBuffer for bitmap
            val intArray = IntArray(imageProxy.width * imageProxy.height)
            for (i in pixels.indices step 4) {
                ...
                intArray[i/4] = (a shl 24) or (r shl 16) or (g shl 8) or b
            }
            bitmap.setPixels(intArray, 0, imageProxy.width, 0, 0, imageProxy.width, imageProxy.height)

            // Create MediaPipe image with rotation
            val mpImage = BitmapImageBuilder(bitmap).build()
            
            // Create processing options with rotation
            val imageProcessingOptions = ImageProcessingOptions.builder()
                .setRotationDegrees(imageProxy.imageInfo.rotationDegrees)
                .build()

            // Process the image with rotation
    ...
    private fun returnLivestreamResult(
        ...
        val landmarks = extractLandmarks(result)

        gestureRecognizerListener?.onResults(
            ResultBundle(
                listOf(result), inferenceTime, input.height, input.width, landmarks
            )
        )
    }
    ...
    private fun extractLandmarks(result: GestureRecognizerResult): List<Float> {
        ...
        if (result.landmarks().isNotEmpty()) {
            ...
            for (landmark in handLandmarks) {
                flattenedLandmarks.add(landmark.x())
                flattenedLandmarks.add(landmark.y())
                flattenedLandmarks.add(landmark.z())
            }
        }
    ...
    companion object {
        ...
        private const val MP_RECOGNIZER_TASK = "gesture_recognizer set11.task"
    ...
    data class ResultBundle(
        ...
        val landmarks: List<Float> = emptyList() // Add this field
    )
